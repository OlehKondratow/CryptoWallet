...
# Usage: ./scripts/RNG_DUMP_QUICK_START.sh
#
# Prerequisites:
...
cd "$REPO_ROOT" || exit 1
FIRMWARE="${REPO_ROOT}/build/cryptowallet.bin"
PORT="${1:-/dev/ttyACM0}"
BAUD="115200"
...
python3 "${REPO_ROOT}/scripts/test_rng_signing_comprehensive.py" --mode rng --port "$PORT"
