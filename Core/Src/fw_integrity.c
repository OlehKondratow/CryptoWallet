...
/* From STM32H743ZITx_FLASH_LWIP.ld */
extern char __app_flash_start;
extern char __app_flash_end;
...
static uint32_t crc32_ieee_update(uint32_t crc, const uint8_t *data, uint32_t len)
{
    ...
    while (len--) {
        crc = (crc >> 8U) ^ table[(crc ^ (uint32_t)(*data++)) & 0xffU];
    }
    return crc;
}
...
{
    s_start = (uint32_t)(uintptr_t)&__app_flash_start;
    s_end = (uint32_t)(uintptr_t)&__app_flash_end;
    if (s_end <= s_start || s_end - s_start > 0x00200000U) {
        s_len = 0U;
        s_crc32 = 0U;
        return;
    }
    ...
    crc = crc32_ieee_update(crc, (const uint8_t *)s_start, s_len);
    s_crc32 = crc ^ 0xFFFFFFFFU;
}
...
int fw_integrity_snprint(char *buf, size_t buf_len)
{
    ...
    return snprintf(buf, buf_len,
                    "FWINFO crc32=0x%08lX,len=%lu,start=0x%08lX,end=0x%08lX",
