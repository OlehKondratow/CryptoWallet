\page crypto_wallet "crypto_wallet: адаптер trezor-crypto (RNG/BIP32/ECDSA)"
...
## Логика потока (RNG + Crypto pipeline)
...
4. Далее каждый вызов `random_buffer()` (из trezor-crypto):
   ...
   - XOR'ит с `s_rng_entropy_pool` и обновляет pool через LCG (мультипликативный конгруэнтный генератор)
   - копирует результат в буфер вывода

### Криптографические пути

**При `USE_CRYPTO_SIGN=1`:**
- BIP-39: mnemonic_from_data (16 байт → 12 слов)
