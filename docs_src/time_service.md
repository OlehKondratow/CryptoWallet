...
`time_service` не участвует в крипто-подписании напрямую: он “обслуживает контекст” (timestamp’ы) для сетевых протоколов, логов и UI. Его бизнес-роль — надёжно стартовать SNTP на стороне LwIP, пережить асинхронность через `tcpip_callback`, сохранить единственную “точку привязки” (epoch_base + tick_base_ms) и затем выдавать корректное “текущее время” даже без повторного обращения к сети.
...
2. Старт клиента SNTP: модуль запускается только один раз (guard `s_sntp_started`), а инициализация выполняется через `tcpip_callback`, чтобы код жил в контексте LwIP.
3. Получение epoch: при синхронизации вызывается callback, в котором сохраняются `s_epoch_base` и `s_tick_base_ms`, и устанавливается `s_time_synced`.
4. В любой момент:
...
### Алгоритм epoch -> UTC
Конвертация выполнена без RTC: через деление на дни/секунды и последовательное вычитание дней по годам и месяцам, с учётом високосных лет.
| Этап | Как считается |
|---|---|
...
- старт SNTP инициируется через `tcpip_callback` (ожидается, что callback выполнится в контексте LwIP TCP/IP thread).
...
- локальные статические флаги/базы: `s_sntp_started`, `s_time_synced`, `s_epoch_base`, `s_tick_base_ms`.

## Связи
- `main.md`: стартует сервис init и инициирует bootstrap.
- `task_net.md` / `app_ethernet_cw.md`: обычно отвечают за момент “после линка” (когда надо вызвать `time_service_start()`).
- `task_display.md`: показывает временные строки/хвосты логов (если они туда передаются).
